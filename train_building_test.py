...
from data_loader import SlippyMapTiles
...
def get_unet_deep():
    ...
    up6 = Conv2DTranspose(filters = 512,kernel_size = (2,2),padding='valid', strides = (2,2), data_format="channels_last")(conv5)
...
train_loader=SlippyMapTiles('/mnt/vol1/PycharmProjects/robosat-master/datasets/training/images','/mnt/vol1/PycharmProjects/robosat-master/datasets/training/labels','/mnt/vol1/Deployment_projects/satellite_image_segmentation/sat_img_crop','/mnt/vol1/Deployment_projects/satellite_image_segmentation/sat_mask_crop')

valid_loader=SlippyMapTiles('/mnt/vol1/PycharmProjects/robosat-master/datasets/validation/images','/mnt/vol1/PycharmProjects/robosat-master/datasets/validation/labels','/mnt/vol1/Deployment_projects/satellite_image_segmentation/sat_val_img','/mnt/vol1/Deployment_projects/satellite_image_segmentation/sat_val_mask')
...
def form_batch(data_loader,batch_size):
    """images is 4D np array of shape [num_images,height,width,num_channels]
       masks is 4D np array of shape [num_images,height,width,mask_channels].
    """
    x_batch = np.zeros((batch_size,img_rows,img_cols,num_channels))
    ...
    for i in range(batch_size):
        # random_width = random.randint(0, images_width - img_cols - 1)
        # random_height = random.randint(0, images_height - img_rows - 1)
        random_image = random.randint(0, len(data_loader) - 1)
        x,y=data_loader.item(random_image)
        y_batch[i] = y
        x_batch[i] = x
    return x_batch, y_batch
...
#
def batch_generator(data_loader,batch_size, horizontal_flip=False, vertical_flip=False, swap_axis=False):
    while True:
        X_batch, y_batch = form_batch(data_loader,batch_size)
...
if __name__ == '__main__':
    ...
    with tf.device('/gpu:0'):
        ...
        # f = h5py.File(os.path.join(data_path, 'train_new.h5'), 'r')
        # X_train = f['train']
        ...
        iters_per_epoch = int(len(train_loader)/ batch_size)
        ...
        #               epochs=nb_epoch,
        #               verbose=1,
        #               validation_data=batch_generator(X_test, y_test, batch_size, horizontal_flip=False, vertical_flip=False, swap_axis=False),
        #               validation_steps = int(X_test.shyape[0]/batch_size), callbacks=callbacks)
        history = model.fit_generator(
            batch_generator(train_loader, batch_size, horizontal_flip=True, vertical_flip=True, swap_axis=True),
            steps_per_epoch=iters_per_epoch,
            epochs=nb_epoch,
            verbose=1,
            validation_data=batch_generator(valid_loader,batch_size, horizontal_flip=False, vertical_flip=False,
                                            swap_axis=False),
            validation_steps=int(len(valid_loader)/ batch_size), callbacks=callbacks)
