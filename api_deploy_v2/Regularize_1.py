...
def generate_step3_results(image_name):
    ...
    for cnt in contours[1]:
        ...
        if (solidity > 0.8 or area<250):
            ...
            f=1
            im = cv2.drawContours(img, [c_hull], 0, (0, 255, 0),thickness=th)
            im_f = cv2.drawContours(f_mask, [c_hull], 0, (0, 0, 0), thickness=2)
            cv2.fillPoly(f_mask, pts=[c_hull], color=(255, 255, 255))
            # print(c_hull)
