...
for root, dirs, files in os.walk("/home/ceinfo/Downloads/tests/images2/",
                                 topdown=False):
    for name in files:
        if os.path.splitext(os.path.join(root, name))[1].lower() == ".jpg":
            x = name[0:-4]
            print("/home/ceinfo/Downloads/results/final_masks(U-net)/" + x + ".png")
            image = cv2.imread("/home/ceinfo/Downloads/results/final_masks(U-net)/" + x + ".png")
            # Read masks
            ...
            cv2.imwrite("/home/ceinfo/Downloads/tests/rectangles(Unet)/" + name, new)
            # cv2.waitKey(0)
