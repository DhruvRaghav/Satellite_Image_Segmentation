...
@app.post('/solar_panel_detection')
async def solar(geotag: str = Form(default=None),
    ...
    try:
        ...
        if not img_type.lower() == 'ortho':
            return JSONResponse(content={"code": "400", "error": "Invalid Image Type",
                                         "message": "Invalid image_type, valid image type are: ortho"}, status_code=400)
        ...
        url = 'http://10.10.21.228:5010/solar_detection_mGIS_aiml'
        url1 = 'http://10.10.21.228:5010/solar_panel_detection'
        url2 = 'http://10.10.21.228:5010/solar_detection_tiff'
        ...
        if geotag == "0":
            ...
            r = requests.request("POST", url2, files=files, headers=headers)
            ...
            r = requests.request("POST", url, files=files, headers=headers)
